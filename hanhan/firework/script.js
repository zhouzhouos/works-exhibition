...
const store = {
	...
	state: {
		...
		paused: true,
		// soundEnabled: initial_param.soundEnabled === 'true' ? true : false,
		soundEnabled: false,
		menuOpen: false,
		openHelpTopic: null,
...
const helpContent = {
	shellType: {
		header: '烟花类型',
		body: '将要发射的烟花类型。选择 “Random” 以获得不错的分类！'
	},
	shellSize: {
		header: '烟花规格',
		body: '烟花的大小。以真实的烟花炮弹大小为模型，较大的炮弹具有更大的爆发和更多的星星，有时效果更复杂。但是较大的规格也需要更多的处理能力，并且可能会导致延迟。'
	},
	quality: {
		header: '动画质量',
		body: '整体图形质量。如果动画运行不流畅，请尝试降低质量。高质量会大大增加渲染的火花量，并可能导致延迟。'
	},
	skyLighting: {
		header: '照亮天空',
		body: '在烟花爆炸时照亮背景。如果背景在屏幕上看起来太亮，请尝试将其设置为“微弱”或“无效”。'
	},
	scaleFactor: {
		header: '动画缩放',
		body: '允许缩放所有烟花的大小，实质上是将视点移近或移远。对于较大尺寸的烟花，稍微减小比例会很方便，尤其是在手机或平板电脑上。'
	},
	autoLaunch: {
		header: '自动发射',
		body: '自动启动烟花序列。坐下来欣赏表演，或禁用以获得完全控制权。'
	},
	finaleMode: {
		header: '多彩多艳',
		body: '发射强烈的烟花。可能会导致滞后。需要启用 “自动发射”。'
	},
	hideControls: {
		header: '隐藏控件',
		body: '隐藏屏幕顶部的半透明控件。对于屏幕截图很有用，或者只是更无缝的体验。隐藏时，您仍然可以点击右上角以重新打开此菜单。'
	},
	fullscreen: {
		header: '全屏视觉',
		body: '占据全屏，获得更完整的视觉体验。'
	},
	longExposure: {
		header: '轨迹留滞',
		body: '保留长条光条纹的实验性效果，类似于让相机快门保持打开状态。'
	}
};
...
function init() {
	...
	setOptionsForSelect(appNodes.quality, [
		{ label: '较低', value: QUALITY_LOW },
		{ label: '常规', value: QUALITY_NORMAL },
		{ label: '较高', value: QUALITY_HIGH }
	]);

	setOptionsForSelect(appNodes.skyLighting, [
		{ label: '无效', value: SKY_LIGHT_NONE },
		{ label: '微弱', value: SKY_LIGHT_DIM },
		{ label: '常规', value: SKY_LIGHT_NORMAL }
	]);
...
// const initial_search = new URLSearchParams(window.location.search)
// const initial_param = {
// 	soundEnabled: initial_search.get('soundEnabled'),
// 	fullscreenEnabled: initial_search.get('fullscreenEnabled')
// }

// console.log(initial_param)


// if (initial_search.get('fullscreenEnabled') === 'true') {
